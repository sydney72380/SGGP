...
class VVAttention(nn.Module):
    ...
    def forward(self, x, attn_mask: Optional[torch.Tensor] = None):
        ...
        if self.logit_scale is not None:
            ...
            attn_ori = torch.bmm(F.normalize(q, dim=-1), F.normalize(k, dim=-1).transpose(-1, -2))
            logit_scale = torch.clamp(self.logit_scale, max=self.logit_scale_max).exp()
            attn_ori = attn_ori.view(N, self.num_heads, L, L) * logit_scale
            attn_ori = attn_ori.view(-1, L, L)
            ...
            # original self-attention
            q = q * self.scale
            attn_ori = torch.bmm(q, k.transpose(-1, -2))
        ...
        if self.head_scale is not None:
            ...
            x = x.view(-1, L, C)
        x_ori = x_ori.transpose(0, 1).reshape(L, N, C)
        x_ori = self.out_proj(x_ori)
        x_ori = self.out_drop(x_ori)
        return x
    def forward(self, x):
        B, N, C = x.shape
